...
export const authorSchema = () => ({
  ...
  // Профили, по которым автора можно опознать как реальную организацию,
  // и ссылка на редакционные принципы: сигналы E-E-A-T и цитируемости.
  sameAs: [SITE.links.instagram, SITE.links.twoGis, SITE.links.yandexMaps],
  publishingPrinciples: absolute(AUTHOR.path),
  knowsAbout: [
    "хостелы Алматы",
    "бюджетное жильё в Алматы",
    "транспорт Алматы",
    "заселение и правила проживания",
  ],
});
...
function AuthorPage() {
  ...
  return (
    ...
    >
      ...
      <AnswerSection title="Как проверяются статьи перед публикацией?">
        <p>
          Каждая статья проходит три механические проверки, и ни одна не публикуется, пока они не
          сойдутся. Первая сравнивает текст со всеми уже вышедшими статьями: если он повторяет
          написанное раньше, статья возвращается на переработку. Вторая считает плотность шаблонных
          оборотов на тысячу слов — порог выставлен по нашим же ранним текстам. Третья смотрит
          структуру: заголовки, блок источников, ссылки.
        </p>
        <p>
          Отдельное правило про факты. Цены, адреса и правила берутся только из наших собственных
          данных, а всё внешнее — расписания, транспорт, порядок регистрации — подтверждается
          ссылкой на первоисточник: сайт аэропорта, метрополитена, автовокзала, портал электронного
          правительства. Каждая ссылка открывается вручную перед публикацией.
        </p>
      </AnswerSection>
